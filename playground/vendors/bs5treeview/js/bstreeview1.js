...
; (function ($, window, document, undefined) {
    ...
    var pluginName = "bstreeview",
        defaults = {
            expandIcon: 'fa fa-chevron-down fa-fw',
            collapseIcon: 'fa fa-chevron-right fa-fw',
            expandClass: 'show',
            indent: 1.25,
                ...
                if (e.target.hasAttribute('href')) {
                    if (_this.settings.openNodeLinkOnNewTab) {
                        ...
                        //alert(e.target.getAttribute('href'));
                           functions_(e.target.getAttribute('href'), 'openfile');

                    }
                    else {
                        //window.location = e.target.getAttribute('href');
                        //alert(e.target.getAttribute('href'));
                        //get_file(e.target.getAttribute('href')).then()
                        functions_(e.target.getAttribute('href'), 'openfile');
                    }
                }
                else
