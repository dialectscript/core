...
function checkTasks_with_AST_JS(code, solution_key, current_lesson_translations) {
  //const solutionKey = Object.keys(current_lesson_translations).find(k => k.endsWith('_SOLUTION'));
  ...
  try {
    ...
    if (expectedCode == '') {
      ...
      const result = compareAST(userAst, solutionAst);
      //console.log(result)
      
      //if failed then try regex just in case
      if(result.match ===false){
        
        let result_regex = checkTasks_with_regex(code, expectedCode);
        if (result_regex.passed ===true){
          return result_regex;
...
function checkTasks_with_AST_AJAMI_(code, solution_key, current_lesson_translations) {
  //const solutionKey = Object.keys(current_lesson_translations).find(k => k.endsWith('_SOLUTION'));
  const solutionKey = solution_key
  if (!solutionKey) return { passed: false, message_key: "No solution defined." };
